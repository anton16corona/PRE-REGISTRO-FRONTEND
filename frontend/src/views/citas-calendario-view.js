...
export class CitasCalendarioView extends LitElement {
  ...
  esDiaFestivo(fecha)          { return DIAS_FESTIVOS.has(fecha); }
  ...
  async obtenerOCrearDia(fecha) {
    ...
    if (data.length > 0) {
      ...
      if (!dia.horarios[0].capacidad) {
        const diaActualizado = { ...dia, horarios: this.generarHorarios(fecha) };
    ...
    if (f.getDay() === 0 || this.esDiaFestivo(fecha)) throw new Error('Día inhábil');

    const nuevoDia = { fecha, horarios: this.generarHorarios(fecha) };
  ...
  async generarCalendarioMes() {
    ...
    for (let i = 1; i <= totalDias; i++) {
      fechas.push({
        dia:   i,
        fecha: new Date(anio, mes, i).toISOString().split('T')[0]
      });
    }
    ...
    fechas.forEach((f, i) => dias.push({ dia: f.dia, fecha: f.fecha, tipo: disponibilidades[i] }));
        ...
        <div class="card">
            ...
            ${this.showHorarios ? html`
              <div class="horarios">
                ...
                ${this.horariosDia.length > 0 ? this.horariosDia.map(hObj => {
                  ...
                  <p style="text-align:center;color:#666;">No hay horarios disponibles para este día</p>
                `}
              </div>
            ` : ''}
          </div>
          ...
          ${this.diaSeleccionado ? html`
            <div class="resumen-seleccion">
              ...
              <strong>Disponibilidad:</strong> ${this.tipoDisponibilidad === 'verde' ? 'Alta' : 'Limitada'}<br>
